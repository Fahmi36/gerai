    ...
    function initialize() {
     ...
     $.ajax({
      ...
      for (let i = 0; i < response.data.length; i++) {
         ...
         if(ukur <= 1.4){
            jarak.push(ukur);
            htmlmitra += '<div class="card card-mitra"><div class="card-body"> <label class="badge text-bg-success" style="float:right;">'+ukur.toFixed(2)+' km</label> <h6 class="fs-14 fw-bold mb-0"><i class="fa fa-map-marker text-danger"></i> '+response.data[i].nama+'</h6><p>'+response.data[i].alamat+'</p><p class="fs-13 mb-0">Sudah Buka</p></div></div>';
